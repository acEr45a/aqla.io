    ...
    <div className="mt-6 flex flex-wrap items-center justify-center gap-x-3 gap-y-2">
      {RANKS.map((r) => (
        <span key={r.key} tabIndex={0}
          className="group relative inline-flex cursor-help items-center gap-2 rounded-full border border-border/70 bg-secondary/40 px-3.5 py-1.5 text-xs text-foreground/90 outline-none focus:border-foreground/30 md:text-sm">
          <span className="h-2.5 w-2.5 rounded-full" style={{ background: r.color, boxShadow: `0 0 10px ${r.color}` }} />
          {r.name}
          <span className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 hidden w-52 -translate-x-1/2 rounded-xl border border-border bg-popover px-3 py-2 text-center text-xs leading-relaxed text-popover-foreground shadow-xl group-hover:block group-focus:block">
            {DESCRIPTIONS[r.key]}
          </span>
        </span>
      ))}
