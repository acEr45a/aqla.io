...
export default function RankLegend() {
  return (
    <div className="mt-4 flex w-full flex-wrap items-center justify-center overflow-visible rounded-xl border border-border/70 bg-secondary/60 px-2 py-2 shadow-xl md:flex-nowrap md:px-4">
      {RANKS.map((r) => (
        <span key={r.key} tabIndex={0}
          className="group relative inline-flex flex-1 cursor-help items-center justify-center px-2 py-1 text-xs outline-none md:text-sm"
          style={{ color: r.color }}>
          {r.name}
